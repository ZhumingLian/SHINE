...
# python evaluation/evaluation_complexcompo.py
def parse_args():
    ...
    parser.add_argument("--results_dir", type=str, default="outputs_complexcompo")
...
def main(args):
    ...
    for benchmark in os.listdir(args.results_dir):
        ...
        for class_name in os.listdir(args.dataset_dir):
            ...
            fg_path = os.path.join(class_path, "fg/03.jpg")
            ...
            with open(content_file, 'r') as f:
                ...
                for item in data:
                    ...
                    result_rect_path = os.path.join(args.results_dir, benchmark, class_name, f"{index}_768_rect.png")
                    if not os.path.exists(result_rect_path):
                        continue
                    result_rect_img = Image.open(result_rect_path)
                    # original bg image
                    bg_rect_path = os.path.join(bg_path, f"{index}_768_rect.png")
                    bg_img = Image.open(bg_rect_path)
                    bg_masked_img = bg_img.copy()
                    draw = ImageDraw.Draw(bg_masked_img)
                    draw.rectangle(bbox, fill=0)
                    # result fg
                    result_fg = result_rect_img.crop(bbox)
                    # result bg
                    result_bg = result_rect_img.copy()
                    draw = ImageDraw.Draw(result_bg)
                    draw.rectangle(bbox, fill=0)
                    # extract fg from ref image to masked_fg
                    ...
                    with torch.no_grad():
                        print(f"Evaluating {result_rect_path}:")

                        # clip new
                        clip_similarity_new = clip_model.calculate_similarity(masked_fg, result_fg)
                        clip_similarity_list.append(clip_similarity_new)
                        print(f"CLIP score: {clip_similarity_new:.4f}")
                        
                        # dinov2 new
                        dinov2_similarity_new = calculate_dinov2_score(dinov2_model, dinov2_processor, masked_fg, result_fg, args.device)
                        dinov2_similarity_list.append(dinov2_similarity_new)
                        print(f"Dinov2 score: {dinov2_similarity_new:.4f}")
                        ...
                        print(f"Dream sim: {distance.item():.4f}")
                        ...
                        # general comparison
                        # result_rect_img.save("result_img.png")

                        # image reward
                        image_reward_score = image_reward_model.score(source_prompt, result_rect_path)
                    ...
                    with open(f"{args.results_dir}/{benchmark}.txt", "a") as f:
                        f.write(f"{result_rect_path}\n")
                        f.write(f"clip:{clip_similarity_new:.4f}\n")
                        f.write(f"dinov2:{dinov2_similarity_new:.4f}\n")
