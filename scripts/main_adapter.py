...
def main(args):
    ...
    if args.use_vlm:
        ...
        editing_pipe = load_editing_model(args)
        if args.enable_model_cpu_offload:
    ...
    instance_prompt = data[0]['instance_prompt'].replace("_", " ")
    ...
    with torch.no_grad():
        ...
        if args.use_vlm:
            prompt = get_vlm_prompt(args, vlm_model, vlm_image_processor, vlm_tokenizer, fill_image_new)
            print(prompt)
            del vlm_model, vlm_image_processor, vlm_tokenizer, fill_pipe
